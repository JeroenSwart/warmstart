...
class BayesianHopt:
    ...
    def visualize_search_performance(
        self, xaxis="iterations", all_losses=False, crossvalidation=False
    ):
        ...
        fig.add_trace(
            go.Scatter(x=idx, y=rolling_min, mode="lines", name="Best so far")
        )
        ...
        if all_losses & crossvalidation:
            ...
            fig.add_trace(
                go.Scatter(
                    x=idx, y=crossval, mode="markers", name="Crossvalidation result"
                )
            )
